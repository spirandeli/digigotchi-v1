# Auditoria Detalhada de Sprites: GARURUMON
...
- **Total de arquivos no diretório**: `336`
- **Total de imagens PNG**: `314`
- **Arquivos não-PNG / Metadados**: `22` (ex: `evolve`, `.gitkeep`)
- **Spritesheets / Folhas Mestre**: `2`
- **Frames individuais (ações montadas)**: `312`
- **Frames brutos em _raw (fatias brutas `sprite_XXXX`)**: `102`
- **Efeitos visuais desacoplados (VFX)**: `0`
- **Projéteis desacoplados**: `0`
- **Arquivos duplicados exatos (SHA-256 idêntico)**: `137`
...
|---|---|---|
| `spritesheets/garurumon.png` | `688x216` | Spritesheet Consolidada |
| `_raw/garurumon.png` | `1254x1254` | Mestre de Referência (_raw) |
...
|---|---|---|---|---:|---|
| `idle` | BOTH | Obrigatório | **COMPLETE** | 20 | 6 unique frames in idle/ |
...
| `signature_attack` | TAMAGOTCHI | Obrigatório | **COMPLETE** | 48 | 7 unique frames for attack-howling-blaster |
...
O personagem possui **137 arquivos duplicados exatos** (SHA-256 idêntico).
...
- **Arquivos envolvidos**: garurumon.png

### Raw Slices in _raw (102 frames)
- **Motivo**: Digimon has 102 raw extracted slices in _raw/ waiting for semantic mapping to Roguelike actions.

## 8. Processamento Recomendado para Completude

1. **Combate Roguelike**: Inspecionar visualmente as 102 fatias em `_raw/` para mapear os índices de `sprite_XXXX.png` para:
...
3. **Manifest**: Criar ou atualizar `docs/digital-path/manifests/garurumon.json` espelhando a estrutura do Veemon (`golden reference`).
