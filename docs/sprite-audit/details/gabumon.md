# Auditoria Detalhada de Sprites: GABUMON
...
- **Total de arquivos no diretório**: `361`
- **Total de imagens PNG**: `339`
- **Arquivos não-PNG / Metadados**: `22` (ex: `evolve`, `.gitkeep`)
- **Spritesheets / Folhas Mestre**: `2`
- **Frames individuais (ações montadas)**: `337`
- **Frames brutos em _raw (fatias brutas `sprite_XXXX`)**: `127`
...
|---|---|---|
| `spritesheets/gabumon.png` | `1002x1130` | Spritesheet Consolidada |
| `_raw/ggabumon.png` | `1447x1087` | Mestre de Referência (_raw) |
...
| `signature_attack` | TAMAGOTCHI | Obrigatório | **COMPLETE** | 48 | 12 unique frames for attack-blue-blaster |
...
- **Arquivos envolvidos**: ggabumon.png

### Raw Slices in _raw (127 frames)
- **Motivo**: Digimon has 127 raw extracted slices in _raw/ waiting for semantic mapping to Roguelike actions.

## 8. Processamento Recomendado para Completude

1. **Combate Roguelike**: Inspecionar visualmente as 127 fatias em `_raw/` para mapear os índices de `sprite_XXXX.png` para:
...
3. **Manifest**: Criar ou atualizar `docs/digital-path/manifests/gabumon.json` espelhando a estrutura do Veemon (`golden reference`).
