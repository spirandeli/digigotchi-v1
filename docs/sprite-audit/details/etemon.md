# Auditoria Detalhada de Sprites: ETEMON
...
- **Total de arquivos no diretório**: `381`
- **Total de imagens PNG**: `359`
- **Arquivos não-PNG / Metadados**: `22` (ex: `evolve`, `.gitkeep`)
- **Spritesheets / Folhas Mestre**: `2`
- **Frames individuais (ações montadas)**: `357`
- **Frames brutos em _raw (fatias brutas `sprite_XXXX`)**: `147`
- **Efeitos visuais desacoplados (VFX)**: `0`
- **Projéteis desacoplados**: `0`
- **Arquivos duplicados exatos (SHA-256 idêntico)**: `136`
...
|---|---|---|
| `spritesheets/etemon.png` | `829x602` | Spritesheet Consolidada |
| `_raw/etemon.png` | `1254x1254` | Mestre de Referência (_raw) |
...
| `play` | TAMAGOTCHI | Obrigatório | **COMPLETE** | 24 | 7 unique frames (total files: 24) |
...
| `signature_attack` | TAMAGOTCHI | Obrigatório | **COMPLETE** | 48 | 10 unique frames for attack-love-serenade |
...
O personagem possui **136 arquivos duplicados exatos** (SHA-256 idêntico).
...
- **Arquivos envolvidos**: etemon.png

### Raw Slices in _raw (147 frames)
- **Motivo**: Digimon has 147 raw extracted slices in _raw/ waiting for semantic mapping to Roguelike actions.

## 8. Processamento Recomendado para Completude

1. **Combate Roguelike**: Inspecionar visualmente as 147 fatias em `_raw/` para mapear os índices de `sprite_XXXX.png` para:
...
3. **Manifest**: Criar ou atualizar `docs/digital-path/manifests/etemon.json` espelhando a estrutura do Veemon (`golden reference`).
