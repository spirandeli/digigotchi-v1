# Auditoria Detalhada de Sprites: METALETEMON
...
- **Total de arquivos no diretório**: `291`
- **Total de imagens PNG**: `269`
- **Arquivos não-PNG / Metadados**: `22` (ex: `evolve`, `.gitkeep`)
- **Spritesheets / Folhas Mestre**: `2`
- **Frames individuais (ações montadas)**: `267`
- **Frames brutos em _raw (fatias brutas `sprite_XXXX`)**: `57`
- **Efeitos visuais desacoplados (VFX)**: `0`
- **Projéteis desacoplados**: `0`
- **Arquivos duplicados exatos (SHA-256 idêntico)**: `134`
...
|---|---|---|
| `spritesheets/metaletemon.png` | `412x419` | Spritesheet Consolidada |
| `_raw/mmetaletemon.png` | `1448x1086` | Mestre de Referência (_raw) |
...
|---|---|---|---|---:|---|
| `idle` | BOTH | Obrigatório | **COMPLETE** | 20 | 4 unique frames in idle/ |
...
| `play` | TAMAGOTCHI | Obrigatório | **COMPLETE** | 24 | 5 unique frames (total files: 24) |
...
| `signature_attack` | TAMAGOTCHI | Obrigatório | **COMPLETE** | 48 | 11 unique frames for attack-banana-slip |
...
O personagem possui **134 arquivos duplicados exatos** (SHA-256 idêntico).
...
- **Arquivos envolvidos**: mmetaletemon.png

### Raw Slices in _raw (57 frames)
- **Motivo**: Digimon has 57 raw extracted slices in _raw/ waiting for semantic mapping to Roguelike actions.

## 8. Processamento Recomendado para Completude

1. **Combate Roguelike**: Inspecionar visualmente as 57 fatias em `_raw/` para mapear os índices de `sprite_XXXX.png` para:
...
3. **Manifest**: Criar ou atualizar `docs/digital-path/manifests/metaletemon.json` espelhando a estrutura do Veemon (`golden reference`).
