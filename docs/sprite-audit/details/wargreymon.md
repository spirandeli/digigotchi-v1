# Auditoria Detalhada de Sprites: WARGREYMON
...
- **Total de arquivos no diretório**: `309`
- **Total de imagens PNG**: `287`
- **Arquivos não-PNG / Metadados**: `22` (ex: `evolve`, `.gitkeep`)
- **Spritesheets / Folhas Mestre**: `2`
- **Frames individuais (ações montadas)**: `285`
- **Frames brutos em _raw (fatias brutas `sprite_XXXX`)**: `75`
...
|---|---|---|
| `spritesheets/wargreymon.png` | `615x635` | Spritesheet Consolidada |
| `_raw/wwargreymon.png` | `1448x1086` | Mestre de Referência (_raw) |
...
| `signature_attack` | TAMAGOTCHI | Obrigatório | **COMPLETE** | 48 | 12 unique frames for attack-terra-force |
...
- **Arquivos envolvidos**: wwargreymon.png

### Raw Slices in _raw (75 frames)
- **Motivo**: Digimon has 75 raw extracted slices in _raw/ waiting for semantic mapping to Roguelike actions.

## 8. Processamento Recomendado para Completude

1. **Combate Roguelike**: Inspecionar visualmente as 75 fatias em `_raw/` para mapear os índices de `sprite_XXXX.png` para:
...
3. **Manifest**: Criar ou atualizar `docs/digital-path/manifests/wargreymon.json` espelhando a estrutura do Veemon (`golden reference`).
