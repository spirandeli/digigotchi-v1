...
//#region node_modules/.nitro/vite/services/ssr/assets/routes-BnkZZGGC.js
	...
	flamedramon: {
		...
		animations: {
			"idle": {
				"fps": 7,
				"loop": true,
				"frames": ["sprites/flamedramon/idle/idle_01.png", "sprites/flamedramon/idle/idle_07.png"]
			},
			"walk_down": {
				"fps": 8,
				"loop": true,
...
function isPathConnected(tiles, start, end, actualWidth, actualHeight) {
	if (tiles[start.y]?.[start.x] !== "floor" || tiles[end.y]?.[end.x] !== "floor") return false;
...
function generateSingleRoom(id, index, kind, seed, biome = "digital", floor = 1, customWidth, customHeight, themeId = "lighting") {
	...
	let shape = "arena";
	if (kind === "boss") shape = "arena";
	else if (kind === "miniboss") shape = rng.pick([
		"arena",
		"central_arena",
		"cross",
		"open"
	]);
	else if (rng.chance(MAP_CONFIG.irregularRoomChance)) shape = rng.pick([
		"arena",
		"corridor",
		"cross",
		"L_shape",
	...
	for (let attempt = 1; attempt <= MAP_CONFIG.maxGenerationAttempts; attempt += 1) {
		...
		if (shape === "arena" || shape === "rectangle" || kind === "boss") for (let y = 1; y < height - 1; y++) for (let x = 1; x < width - 1; x++) tiles[y][x] = "floor";
		else if (shape === "corridor") {
			const cy = Math.floor(height / 2);
			for (let y = cy - 2; y <= cy + 2; y++) for (let x = 1; x < width - 1; x++) tiles[y][x] = "floor";
			const alcoveX1 = Math.floor(width * .3);
			const alcoveX2 = Math.floor(width * .7);
			for (let y = 1; y < height - 1; y++) for (let dx = -1; dx <= 1; dx++) {
		...
		} else if (shape === "open" || shape === "pillars_arena") {
			for (let y = 1; y < height - 1; y++) for (let x = 1; x < width - 1; x++) tiles[y][x] = "floor";
			const pCols = [Math.floor(width * .28), Math.floor(width * .72) - 1];
			const pRows = [Math.floor(height * .3), Math.floor(height * .7) - 1];
			for (const py of pRows) for (const px of pCols) for (let dy = 0; dy < 2; dy++) for (let dx = 0; dx < 2; dx++) if (py + dy > 1 && py + dy < height - 2 && px + dx > 1 && px + dx < width - 2) tiles[py + dy][px + dx] = "wall";
		} else if (shape === "compact") {
			const padY = Math.max(1, Math.floor(height * .15));
			const padX = Math.max(1, Math.floor(width * .1));
			for (let y = padY; y < height - padY; y++) for (let x = padX; x < width - padX; x++) tiles[y][x] = "floor";
		} else if (shape === "multi_room" || shape === "divided_chambers") {
			for (let y = 1; y < height - 1; y++) for (let x = 1; x < width - 1; x++) tiles[y][x] = "floor";
			const midX = Math.floor(width / 2);
			const doorY = Math.floor(height / 2);
			for (let y = 1; y < height - 1; y++) if (Math.abs(y - doorY) > 1) tiles[y][midX] = "wall";
		} else if (shape === "asymmetric") {
			for (let y = 1; y < height - 1; y++) for (let x = 1; x < width - 1; x++) tiles[y][x] = "floor";
			for (let y = 1; y < Math.floor(height * .4); y++) for (let x = 1; x < Math.floor(width * .25); x++) tiles[y][x] = "wall";
			for (let y = Math.floor(height * .65); y < height - 1; y++) for (let x = Math.floor(width * .75); x < width - 1; x++) tiles[y][x] = "wall";
		} else if (shape === "winding") {
			for (let y = 1; y < height - 1; y++) for (let x = 1; x < width - 1; x++) tiles[y][x] = "floor";
			const midX = Math.floor(width / 2);
			const h3 = Math.floor(height / 3);
			for (let y = 1; y < h3 * 2; y++) tiles[y][Math.floor(midX * .7)] = "wall";
			for (let y = h3; y < height - 1; y++) tiles[y][Math.floor(midX * 1.3)] = "wall";
		} else if (shape === "chokepoint") {
			for (let y = 1; y < height - 1; y++) for (let x = 1; x < width - 1; x++) tiles[y][x] = "floor";
			const midX = Math.floor(width / 2);
			const cy = Math.floor(height / 2);
			for (let y = 1; y < height - 1; y++) if (Math.abs(y - cy) > 1) {
				tiles[y][midX] = "wall";
				tiles[y][midX - 1] = "wall";
			}
			...
			const cy = Math.floor(height / 2);
			for (let dy = -1; dy <= 1; dy++) for (let dx = -2; dx <= 1; dx++) tiles[cy + dy][cx + dx] = "wall";
		}
		...
		if (!isPathConnected(tiles, spawn, exit, width, height)) carveCorridor(tiles, spawn.x, spawn.y, exit.x, exit.y, corridorWidth, width, height);
		...
		const combatNavMask = /* @__PURE__ */ new Set();
		for (let dy = -3; dy <= 3; dy++) for (let dx = -3; dx <= 3; dx++) {
			combatNavMask.add(`${spawn.x + dx},${spawn.y + dy}`);
			combatNavMask.add(`${exit.x + dx},${exit.y + dy}`);
		}
		const midY = Math.floor(height / 2);
		for (let x = spawn.x; x <= exit.x; x++) for (let dy = -1; dy <= 1; dy++) combatNavMask.add(`${x},${midY + dy}`);
		...
		if (reachableTiles.some((t) => t.x === exit.x && t.y === exit.y) && reachableTiles.length >= 25) {
			...
			if (kind === "boss" || kind === "event" || sizeCategory === "arena") {
				...
				const lmY = Math.max(2, Math.floor(height * .22));
				landmark = {
					id: `${id}_landmark`,
			...
			const availableClusterTiles = reachableTiles.filter((t) => !combatNavMask.has(`${t.x},${t.y}`) && Math.hypot(t.x - spawn.x, t.y - spawn.y) >= 4 && Math.hypot(t.x - exit.x, t.y - exit.y) >= 4);
			const clusterTypes = themeId === "fire" ? [
				"rubble",
				"hazard",
				"structure"
			] : themeId === "ice" ? [
			...
			const sortedForProp = reachableTiles.filter((t) => Math.hypot(t.x - spawn.x, t.y - spawn.y) >= 4 && Math.hypot(t.x - exit.x, t.y - exit.y) >= 3).sort((a, b) => Math.hypot(a.x - idealCenterX, a.y - idealCenterY) - Math.hypot(b.x - idealCenterX, b.y - idealCenterY));
			const centerPropTile = sortedForProp[0] || {
				x: idealCenterX,
				y: idealCenterY
			};
			...
			if (kind === "miniboss") {
				const mbChestTile = sortedForProp[sortedForProp.length - 1] || {
					x: exit.x - 2,
					y: exit.y
				};
				props.push({
			...
			if (deadEndPropX > 0 && deadEndPropY > 0 && props.length === 0 && kind !== "boss" && kind !== "miniboss") {
				if (reachableTiles.some((t) => t.x === deadEndPropX && t.y === deadEndPropY)) props.push({
					id: `${id}_pocket_chest`,
					type: "chest",
					tileX: deadEndPropX,
					tileY: deadEndPropY
			...
			if (kind === "boss") {
				...
				const bossStats = calculateEnemyStats(bossDef.baseHp, bossDef.attackDamage, bossDef.defense, bossDef.speed, 1600, bossDef.xpReward, bossDef.coinReward, floor, true, false);
				enemies.push({
					id: `${id}_boss_1`,
				...
				const mbStats = calculateEnemyStats(mbDef.baseHp, mbDef.attackDamage, mbDef.defense, mbDef.speed, 1500, mbDef.xpReward, mbDef.coinReward, floor, false, true);
				...
				const shuffled = [...reachableTiles.filter((t) => Math.hypot(t.x - spawn.x, t.y - spawn.y) >= 5 && Math.hypot(t.x - exit.x, t.y - exit.y) >= 2.5 && !props.some((p) => p.tileX === t.x && p.tileY === t.y))].sort(() => rng.next() - .5);
